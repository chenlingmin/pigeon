...
public class RegionServiceImpl implements RegionService {
    ...
    public List<RegionResponse> list(RegionRequest regionRequest) {
        ...
        return regions.stream().map(region -> region.convert(RegionResponse.class)).collect(Collectors.toList());
    }
    ...
    public List<RegionResponse> queryTree() {
        List<RegionEntity> regions = regionRepository.findByParentNo(NO_PARENT_ID);
        return regions.stream().map(region -> queryTree(region.convert(RegionResponse.class), DISTRICT_LEVEL)).collect(Collectors.toList());
    }
        ...
        if (regionResponse.getLevel() < level) { // 区级
            ...
            regionResponse = regionResponse.setRegionList(
                    regions.stream().map(region ->
                            queryTree(region.convert(RegionResponse.class), DISTRICT_LEVEL)).collect(Collectors.toList()));
        }
